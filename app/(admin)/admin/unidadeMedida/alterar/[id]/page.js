...
export default function GerenciarUnidadeMedidaAdmin() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const url = isEditing
                ? `/api/admin/unidadeMedida/${id_uni_medida}`
                : '/api/admin/unidadeMedida';
            ...
            if (!response.ok) {
                throw new Error(result.error || `Erro ao ${isEditing ? 'atualizar' : 'cadastrar'} unidade de medida`);
            }

            setSuccess('Unidade de medida atualizado com sucesso!');
            setTimeout(() => {
                if (isEditing) {
            ...
            setError(err.message || `Erro ao ${isEditing ? 'atualizar' : 'cadastrar'} unidade de medida`);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="usuario-admin-container">
            ...
            <section className="usuario-cadastro-admin">
                <div className="header-actions">
                    <h1>{isEditing ? 'Editar Unidade de Medida' : 'Cadastrar Unidade de Medida'} #{id_uni_medida}</h1>
