...
export default function UsuariosAdmin() {
  ...
  const fetchUsuarios = async (page = 1) => {
    try {
      setLoading(true);
      const res = await fetch(`/api/usuarios/admin/novos/?page=${page}&perPage=${pagination.perPage}`);
      ...
      console.error('Erro ao carregar os usuários novos:', err);
  ...
  if (error) {
    return (
      <div className="error-container">
        <p className="error-message">Erro ao carregar usuários novos: {error}</p>
        <button 
      ...
      <div className="usuario-lista-admin">
        ...
        <div className="usuario-admin-box">
          {usuarios.length > 0 ? (
            ...
            <p className="usuario-empty">Nenhum usuário novo encontrado.</p>
          )}
        </div>
