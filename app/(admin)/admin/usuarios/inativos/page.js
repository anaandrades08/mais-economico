...
export default function UsuariosAdmin() {
  ...
  const fetchUsuarios = async (page = 1) => {
    try {
      setLoading(true);
      const res = await fetch(`/api/usuarios/admin/inativos/?page=${page}&perPage=${pagination.perPage}`);
      
      if (!res.ok) {
        throw new Error(`Erro ao carregar usuários inativos: ${res.statusText}`);
      }
      ...
      console.error('Erro ao carregar os usuários inativos:', err);
  ...
  if (error) {
    return (
      <div className="error-container">
        <p className="error-message">Erro ao carregar usuários inativos: {error}</p>
        <button 
  ...
  return (
    <div className="usuario-admin-container">
      ...
      <div className="usuario-lista-admin">
        <h1>Lista de Usuários - Inativos</h1>
            ...
            usuarios.map(usuario => (
              ...
              > 
                ...
                <p className="usuario-ativo">
                  Ativo: {usuario.ativo === 2 ? "Reprovado" :
                         usuario.ativo === 1 ? "Sim" :
                         usuario.ativo === 0 ? "Não" :
