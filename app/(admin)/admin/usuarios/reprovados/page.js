...
export default function UsuariosAdmin() {
  ...
  const fetchUsuarios = async (page = 1) => {
    try {
      setLoading(true);
      const res = await fetch(`/api/usuarios/admin/pendentes/?page=${page}&perPage=${pagination.perPage}`);
  ...
  if (error) {
    return (
      <div className="error-container">
        <p className="error-message">Erro ao carregar usuários pendente: {error}</p>
        <button 
  ...
  return (
    <div className="usuario-admin-container">
      ...
      <div className="usuario-lista-admin">
        <h1>Lista de Usuários - Pendentes</h1>
        ...
        <div className="usuario-admin-box">
          {usuarios.length > 0 ? (
            ...
            <p className="usuario-empty">Nenhum usuário pendente encontrado.</p>
          )}
        </div>
