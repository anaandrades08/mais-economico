    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isDeleting) {
                ...
                if (!response.ok) {
                    throw new Error(result.error || 'Erro ao atualizar tipo de ingrediente');
                }

                setSuccess('Tipo de ingrediente atualizado com sucesso!');
            } else {
                // Cadastro
                ...
                if (!response.ok) {
                    throw new Error(result.error || 'Erro ao cadastrar tipo de ingrediente');
                }

                setSuccess('Tipo de ingrediente cadastrado com sucesso!');
                limparFormulario();
            }
        } catch (err) {
            console.error('Erro ao salvar tipo de ingrediente:', err);
            setError(err.message || 'Erro ao salvar tipo de ingrediente');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="usuario-admin-container">
            ...
            <section className="usuario-cadastro-admin">
                <div className="header-actions">
                    <h1>{isDeleting ? 'Excluir Tipo de Ingrediente' : 'Cadastrar Tipo de Ingrediente'} #{id_tipo_ingrediente}</h1>
