...
export default function ReceitasComIngredientesAdmin() {
    ...
    const fetchReceitasComIngredientes = async (page = 1) => {
        try {
            setLoading(true);
            const response = await fetch(
                `/api/admin/receitas/com-modo-preparo?page=${page}&perPage=${pagination.perPage}`
            );
            ...
            if (!result.success) {
                throw new Error(result.error || 'Erro ao carregar receitas com ingredientes');
            }
            ...
            console.error('Erro ao carregar receitas com ingredientes:', err);
    ...
    const handlePageChange = (newPage) => {
        ...
        fetchReceitasComIngredientes(newPage);
        window.scrollTo({ top: 0, behavior: 'smooth' });
    };

    useEffect(() => {
        fetchReceitasComIngredientes(pagination.page);
    }, []);
    ...
    if (loading && totalReceitas === 0) {
        return (
            <div className="loading-container">
                ...
                <p>Carregando receitas com modo de preparo...</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="error-container">
                ...
                <button onClick={() => fetchReceitasComIngredientes(pagination.page)}>
                    Tentar novamente
                </button>
            </div>
        );
    ...
    return (
        <div className="usuario-admin-container">
            ...
            <div className="usuario-lista-admin">
                <h1>Receitas com Modo de preparo ({totalReceitas})</h1>
                ...
                {totalReceitas > 0 ? (
                        ...
                        <div className="receitas-grid">
                            {receitas.map(receita => (
                                <div key={receita.id_receita} className="receita-card">
                                    <Link href={`/admin/modo-preparo-receita/receita/${receita.id_receita}`}>
                                        <div className="receita-image-container">
                                            ...
                                            <div className="feedback-count">
                                                <GiCook /> {receita._count?.titulosPreparo || 0}
                                            </div>
                                        </div>
                    ...
                    <div className="empty-message">
                        Nenhuma receita com modo de preparo encontrada.
                    </div>
                )}
