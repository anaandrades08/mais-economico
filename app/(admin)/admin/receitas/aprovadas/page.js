...
export default function ReceitasAdmin() {
    ...
    const filter = searchParams.get('filter') || 'aprovadas'; // Padrão para 'novo' se não houver filtro
    ...
    const fetchReceitas = async (page = 1) => {
        try {
            ...
            const { success, data, pagination: paginationData, error } = await response.json();
            ...
            setReceitas(data || []);
            setTotalReceitas(receitas.length);
